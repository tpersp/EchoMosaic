...
# creates a local virtual environment, and manages the appropriate systemd service.
...
run_privileged() {
  ...
  echo "Run install.sh as root or install/configure sudo for ${SERVICE_USER}." >&2
  return 1
}
...
run_privileged apt-get install < /dev/null -y python3 python3-venv python3-pip ffmpeg git

update_config_path() {
  local config_key="$1"
  local target_path="$2"
  python3 - "$INSTALL_DIR" "$config_key" "$target_path" "$SERVICE_NAME" "$SERVICE_SCOPE" "$BRANCH" "$UPDATE_CHANNEL" "$REPO_SLUG" <<'PY'
import json
import sys
from pathlib import Path
...
service_scope = sys.argv[5]
branch = sys.argv[6]
update_channel = sys.argv[7]
repo_slug = sys.argv[8]
...
data["SERVICE_SCOPE"] = service_scope
...
write_install_metadata() {
  python3 - "$INSTALL_DIR" "$BRANCH" "$UPDATE_CHANNEL" "$REPO_SLUG" "$SERVICE_NAME" "$SERVICE_SCOPE" <<'PY'
import json
import subprocess
import sys
from pathlib import Path
...
ensure_cifs_utils() {
  if ! dpkg -s cifs-utils >/dev/null 2>&1; then
    ...
    run_privileged apt-get update && run_privileged apt-get install < /dev/null -y cifs-utils
  fi
}
...
configure_library_path() {
  ...
  if [[ "$mount_answer" =~ ^[Yy]$ ]]; then
    ...
    if [[ "$AUTH_CHOICE" == "2" ]]; then
      ...
      run_privileged mkdir -p "$(dirname "$CRED_FILE")"
      if [[ -n "$CIFS_DOMAIN" ]]; then
        printf "username=%s\npassword=%s\ndomain=%s\n" "$CIFS_USER" "$CIFS_PASS" "$CIFS_DOMAIN" | run_privileged tee "$CRED_FILE" >/dev/null
      else
        printf "username=%s\npassword=%s\n" "$CIFS_USER" "$CIFS_PASS" | run_privileged tee "$CRED_FILE" >/dev/null
      fi
      run_privileged chmod 600 "$CRED_FILE"
    ...
    run_privileged chown "$SERVICE_USER:$SERVICE_USER" "$MOUNT_POINT"
    ...
    if grep -qsF "$SERVER_SHARE  $MOUNT_POINT  cifs" /etc/fstab; then
      ...
      echo "  $FSTAB_LINE"
      echo "$FSTAB_LINE" | run_privileged tee -a /etc/fstab >/dev/null
    fi
    echo "Mounting $MOUNT_POINT..."
    if ! run_privileged mount "$MOUNT_POINT"; then
...
echo -e "\nCreating systemd ${SERVICE_SCOPE} service..."
if [ "$SERVICE_SCOPE" = "system" ]; then
  LEGACY_USER_SERVICE_PATH="$HOME/.config/systemd/user/${SERVICE_NAME}"
  if [ -f "$LEGACY_USER_SERVICE_PATH" ]; then
    echo "Migrating the existing root user service to a system service..."
    systemctl --user stop "$SERVICE_NAME" 2>/dev/null || true
    systemctl --user disable "$SERVICE_NAME" 2>/dev/null || true
    rm -f "$LEGACY_USER_SERVICE_PATH"
    systemctl --user daemon-reload 2>/dev/null || true
    systemctl --user reset-failed 2>/dev/null || true
  fi
  SERVICE_DIR="/etc/systemd/system"
  SERVICE_TARGET="multi-user.target"
  SERVICE_USER_DIRECTIVE="User=$SERVICE_USER"
  SERVICE_HOME_DIRECTIVE="Environment=\"HOME=$HOME\""
else
  SERVICE_DIR="$HOME/.config/systemd/user"
  SERVICE_TARGET="default.target"
  SERVICE_USER_DIRECTIVE=""
  SERVICE_HOME_DIRECTIVE=""
fi
...
printf '%s\n' "$SERVICE_CONTENT" | run_privileged tee "$SERVICE_PATH" >/dev/null

echo -e "\nEnabling and starting systemd ${SERVICE_SCOPE} service..."
if [ "$SERVICE_SCOPE" = "system" ]; then
  run_privileged systemctl daemon-reload
  run_privileged systemctl enable "$SERVICE_NAME"
  run_privileged systemctl restart "$SERVICE_NAME"
  SYSTEMCTL_CMD="systemctl"
  JOURNALCTL_CMD="journalctl -u"
else
  systemctl --user daemon-reload
  systemctl --user enable "$SERVICE_NAME"
  systemctl --user restart "$SERVICE_NAME"
  if ! loginctl show-user "$SERVICE_USER" | grep -q "Linger=yes"; then
    echo "Enabling linger for user $SERVICE_USER so the service runs on boot..."
    run_privileged loginctl enable-linger "$SERVICE_USER" || true
  fi
  SYSTEMCTL_CMD="systemctl --user"
  JOURNALCTL_CMD="journalctl --user -u"
fi
...
echo "  $JOURNALCTL_CMD $SERVICE_NAME -n 100 --no-pager"
