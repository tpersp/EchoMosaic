...
def test_playback_engine_retries_after_no_media_and_recovers() -> None:
    emitted = []
    runtime_updates = []
    items = []
    ...
    manager = StreamPlaybackManager(
        safe_emit=lambda event, payload, to=None: emitted.append((event, payload, to)),
        list_media=lambda folder, library="media": list(items),
        library_for_media_mode=lambda mode: "media",
        update_stream_runtime_state=lambda stream_id, **kwargs: runtime_updates.append((stream_id, kwargs)) or {"url": "thumb"},
        get_runtime_thumbnail_payload=lambda stream_id: {"url": "thumb"},
        get_sync_timer_config=lambda timer_id: None,
        ...
        stream_update_event="stream_update",
        sync_time_event="sync_time",
        stream_sync_interval_seconds=0.25,
        sync_switch_lead_seconds=0.25,
        stream_no_media_retry_seconds=1.0,
    )
    try:
        manager.bootstrap(
            {
        ...
        assert payload["error"] == "no_media"
        assert payload["next_update_at"] is not None

        items.append({"path": "root/photo.jpg", "kind": "image"})
        recovered = manager.ensure_started("stream1")
        assert recovered is not None
        assert recovered["status"] == "playing"
        assert recovered["media"]["path"] == "root/photo.jpg"
    finally:
        manager.stop()


def test_playback_engine_shuffle_avoids_recent_history_when_possible() -> None:
    emitted = []
    runtime_updates = []
    items = [
        {"path": "root/a.jpg", "kind": "image"},
        {"path": "root/b.jpg", "kind": "image"},
        {"path": "root/c.jpg", "kind": "image"},
        {"path": "root/d.jpg", "kind": "image"},
    ]
    ...
    manager = StreamPlaybackManager(
        ...
        sync_supported_media_modes={"image"},
        stream_playback_history_limit=50,
        stream_random_recent_avoid_count=3,
        stream_update_event="stream_update",
        sync_time_event="sync_time",
        stream_sync_interval_seconds=3.0,
        sync_switch_lead_seconds=0.25,
    ...
    try:
        ...
        with manager._lock:
            state = manager._states["stream1"]
            state.current_media = {"path": "root/d.jpg", "kind": "image"}
            state.history = [
                {"media": {"path": "root/a.jpg", "kind": "image"}},
                {"media": {"path": "root/b.jpg", "kind": "image"}},
                {"media": {"path": "root/c.jpg", "kind": "image"}},
            ]
            choice = manager._next_media(state)
        assert choice is not None
        assert choice["path"] == "root/a.jpg"
    finally:
        manager.stop()
